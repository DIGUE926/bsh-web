...
export default async function ClassementJoueursPPGPage({
  params,
}: {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <Breadcrumb
        items={[{ label: league.name, href: `/${slug}` }, { label: "Classement joueurs — PPG" }]}
      />
      <h1 className="font-display text-2xl text-bsh-orange mb-1 tracking-wide">
        CLASSEMENT {league.name.toUpperCase()} — PPG
      </h1>
      <p className="text-white/50 mb-6">
        Meilleurs en points par match
      </p>
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="border-b border-white/20 text-white/50 uppercase">
                ...
                <th className="py-2 px-2 text-center text-bsh-orange">PPG</th>
              </tr>
            </thead>
            <tbody>
